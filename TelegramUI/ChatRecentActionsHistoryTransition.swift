    ...
    func item(account: Account, peer: Peer, controllerInteraction: ChatControllerInteraction) -> ListViewItem {
        switch self.entry.event.action {
            case let .changeTitle(_, new):
                ...
                return ChatMessageItem(presentationData: self.presentationData, account: account, chatLocation: .peer(peer.id), controllerInteraction: controllerInteraction, content: .message(message: message, read: true, selection: .none))
            case let .changeAbout(prev, new):
                ...
                switch self.id.contentIndex {
                    ...
                    case .content:
                        ...
                        return ChatMessageItem(presentationData: self.presentationData, account: account, chatLocation: .peer(peer.id), controllerInteraction: controllerInteraction, content: .message(message: message, read: true, selection: .none), additionalContent: !prev.isEmpty ? .eventLogPreviousDescription(prevMessage) : nil)
                }
            case let .changeUsername(prev, new):
                ...
                switch self.id.contentIndex {
                    ...
                    case .content:
                        ...
                        return ChatMessageItem(presentationData: self.presentationData, account: account, chatLocation: .peer(peer.id), controllerInteraction: controllerInteraction, content: .message(message: message, read: true, selection: .none), additionalContent: !prev.isEmpty ? .eventLogPreviousLink(prevMessage) : nil)
                }
            ...
            case let .editMessage(prev, message):
                switch self.id.contentIndex {
                    case .header:
                        ...
                        appendAttributedText(text: self.presentationData.strings.Channel_AdminLog_MessageEdited(author?.displayTitle ?? ""), generateEntities: { index in
                            if index == 0, let author = author {
                                return [.TextMention(peerId: author.id)]
                            }
                            return []
                    ...
                    case .content:
                        ...
                        let message = Message(stableId: self.entry.stableId, stableVersion: 0, id: MessageId(peerId: peer.id, namespace: Namespaces.Message.Cloud, id: Int32(bitPattern: self.entry.stableId)), globallyUniqueId: self.entry.event.id, groupingKey: nil, groupInfo: nil, timestamp: self.entry.event.date, flags: [.Incoming], tags: [], globalTags: [], localTags: [], forwardInfo: nil, author: message.author, text: message.text, attributes: attributes, media: message.media, peers: peers, associatedMessages: SimpleDictionary(), associatedMessageIds: [])
                        return ChatMessageItem(presentationData: self.presentationData, account: account, chatLocation: .peer(peer.id), controllerInteraction: controllerInteraction, content: .message(message: filterOriginalMessageFlags(message), read: true, selection: .none), additionalContent: !prev.text.isEmpty || !message.text.isEmpty ? .eventLogPreviousMessage(filterOriginalMessageFlags(prev)) : nil)
                }
...
func chatRecentActionsHistoryPreparedTransition(from fromEntries: [ChatRecentActionsEntry], to toEntries: [ChatRecentActionsEntry], type: ChannelAdminEventLogUpdateType, canLoadEarlier: Bool, displayingResults: Bool, account: Account, peer: Peer, controllerInteraction: ChatControllerInteraction) -> ChatRecentActionsHistoryTransition {
    ...
    return ChatRecentActionsHistoryTransition(filteredEntries: toEntries, type: type, deletions: deletions, insertions: insertions, updates: updates, canLoadEarlier: canLoadEarlier, displayingResults: displayingResults)
}
