...
class ChatMessageStickerItemNode: ChatMessageItemView {
    ...
    override func asyncLayout() -> (_ item: ChatMessageItem, _ params: ListViewItemLayoutParams, _ mergedTop: ChatMessageMerge, _ mergedBottom: ChatMessageMerge, _ dateHeaderAtBottom: Bool) -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation) -> Void) {
        ...
        return { item, params, mergedTop, mergedBottom, dateHeaderAtBottom in
            ...
            let imageFrameSize = CGSize(width: imageSize.width + 10.0 * 2.0, height: imageSize.height + 10.0 * 2.0)
            let imageFrame = CGRect(origin: CGPoint(x: 0.0 + (incoming ? (params.leftInset + layoutConstants.bubble.edgeInset + avatarInset + layoutConstants.bubble.contentInsets.left) : (params.width - params.rightInset - imageFrameSize.width - layoutConstants.bubble.edgeInset - layoutConstants.bubble.contentInsets.left)), y: 0.0), size: CGSize(width: imageFrameSize.width + 0.0, height: imageFrameSize.height + 0.0))
            
            let arguments = TransformImageArguments(corners: ImageCorners(), imageSize: imageSize, boundingSize: CGSize(width: imageSize.width, height: imageSize.height), intrinsicInsets: UIEdgeInsets(top: 10.0, left: 10.0, bottom: 10.0, right: 10.0))
