...
final class YoutubeEmbedImplementation: WebEmbedImplementation {
    ...
    init(videoId: String, timestamp: Int = 0) {
        ...
        self.status = MediaPlayerStatus(generationTimestamp: 0.0, duration: 0.0, dimensions: CGSize(), timestamp: Double(timestamp), seekId: 0, status: .buffering(initial: true, whilePlaying: true))
    }
            ...
            case "onState":
                ...
                if let components = URLComponents(url: url, resolvingAgainstBaseURL: false) {
                    ...
                    if let updateStatus = self.updateStatus, let playback = playback, let duration = duration {
                        ...
                        self.status = MediaPlayerStatus(generationTimestamp: self.status.generationTimestamp, duration: Double(duration), dimensions: self.status.dimensions, timestamp: newTimestamp, seekId: 0, status: playbackStatus)
                        updateStatus(self.status)
                    }
                }
