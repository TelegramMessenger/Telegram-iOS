...
extension CATransform3D {
  ...
  static func makeTransform(
    ...
    -> CATransform3D
  {
    let result: CATransform3D
    if let skew = skew, let skewAxis = skewAxis {
      result = CATransform3DMakeTranslation(position.x, position.y, 0).rotated(rotation).skewed(skew: -skew, skewAxis: skewAxis)
        .scaled(scale * 0.01).translated(anchor * -1)
    } else {
      result = CATransform3DMakeTranslation(position.x, position.y, 0).rotated(rotation).scaled(scale * 0.01).translated(anchor * -1)
    }
