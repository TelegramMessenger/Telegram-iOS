        ...
        return { item, layoutConstants, position, _ in
            return (CGFloat.greatestFiniteMagnitude, { constrainedSize in
                ...
                if let statusType = statusType {
                    let (size, apply) = statusLayout(dateText: dateText, type: statusType, constrainedSize: textConstrainedSize)
                    statusSize = size
                    statusApply = apply
                }
                ...
                let (textLayout, textApply) = textLayout(attributedString: attributedText, backgroundColor: nil, maximumNumberOfLines: 0, truncationType: .end, constrainedSize: textConstrainedSize, cutout: nil)
