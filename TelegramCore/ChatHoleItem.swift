...
class ChatHoleItemNode: ListViewItemNode {
    ...
    func asyncLayout() -> (width: CGFloat) -> (ListViewItemNodeLayout, () -> Void) {
        ...
        return { width in
            let (size, apply) = labelLayout(attributedString: NSAttributedString(string: "Loading", font: titleFont, textColor: UIColor.white), backgroundColor: nil, maximumNumberOfLines: 1, truncationType: .end, constrainedSize: CGSize(width: width, height: CGFloat.greatestFiniteMagnitude), cutout: nil)
