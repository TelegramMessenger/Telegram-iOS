...
final class ChatMessageInteractiveFileNode: ASTransformNode {
    ...
    func asyncLayout() -> (account: Account, file: TelegramMediaFile, incoming: Bool, constrainedSize: CGSize) -> (CGFloat, (CGSize) -> (CGFloat, (CGFloat) -> (CGSize, () -> Void))) {
        ...
        return { account, file, incoming, constrainedSize in
            return (CGFloat.greatestFiniteMagnitude, { constrainedSize in
                ...
                let (titleLayout, titleApply) = titleAsyncLayout(attributedString: titleString, backgroundColor: nil, maximumNumberOfLines: 1, truncationType: .middle, constrainedSize: textConstrainedSize, cutout: nil)
                let (descriptionLayout, descriptionApply) = descriptionAsyncLayout(attributedString: descriptionString, backgroundColor: nil, maximumNumberOfLines: 1, truncationType: .middle, constrainedSize: textConstrainedSize, cutout: nil)
    ...
    static func asyncLayout(_ node: ChatMessageInteractiveFileNode?) -> (account: Account, file: TelegramMediaFile, incoming: Bool, constrainedSize: CGSize) -> (CGFloat, (CGSize) -> (CGFloat, (CGFloat) -> (CGSize, () -> ChatMessageInteractiveFileNode))) {
        let currentAsyncLayout = node?.asyncLayout()
        
        return { account, file, incoming, constrainedSize in
            var fileNode: ChatMessageInteractiveFileNode
            var fileLayout: (account: Account, file: TelegramMediaFile, incoming: Bool, constrainedSize: CGSize) -> (CGFloat, (CGSize) -> (CGFloat, (CGFloat) -> (CGSize, () -> Void)))
            ...
            let (initialWidth, continueLayout) = fileLayout(account: account, file: file, incoming: incoming, constrainedSize: constrainedSize)
