...
class ChatListItem: ListViewItem {
    ...
    init(account: Account, message: Message, unreadCount: Int, action: (Message) -> Void) {
        ...
        if let node = node as? ChatListItemNode {
            Queue.mainQueue().async {
                ...
                async {
                    ...
                    let (nodeLayout, apply) = layout(account: self.account, width: width, first: first, last: last)
                    Queue.mainQueue().async {
...
class ChatListItemNode: ListViewItemNode {
    ...
    override func layoutForWidth(_ width: CGFloat, item: ListViewItem, previousItem: ListViewItem?, nextItem: ListViewItem?) {
        ...
        let (_, apply) = layout(account: self.account, width: width, first: self.relativePosition.first, last: self.relativePosition.last)
        apply()
    }
    ...
    func asyncLayout() -> (account: Account?, width: CGFloat, first: Bool, last: Bool) -> (ListViewItemNodeLayout, () -> Void) {
        ...
        return { account, width, first, last in
            ...
            if let message = message {
                ...
                var messageText: NSString = message.text
                ...
                if let author = message.author as? TelegramUser, let peer = peer, peer as? TelegramUser == nil {
                    let peerText: NSString = author.id == account?.peerId ? "You: " : author.compactDisplayTitle + ": "
            ...
            let (dateLayout, dateApply) = dateLayout(attributedString: dateAttributedString, backgroundColor: nil, maximumNumberOfLines: 1, truncationType: .end, constrainedSize: CGSize(width: contentRect.width, height: CGFloat.greatestFiniteMagnitude), cutout: nil)
            
            let (badgeLayout, badgeApply) = badgeTextLayout(attributedString: badgeAttributedString, backgroundColor: nil, maximumNumberOfLines: 1, truncationType: .end, constrainedSize: CGSize(width: 50.0, height: CGFloat.greatestFiniteMagnitude), cutout: nil)
            ...
            let (textLayout, textApply) = textLayout(attributedString: textAttributedString, backgroundColor: nil, maximumNumberOfLines: 1, truncationType: .end, constrainedSize: CGSize(width: contentRect.width - badgeSize, height: CGFloat.greatestFiniteMagnitude), cutout: nil)
            ...
            let (titleLayout, titleApply) = titleLayout(attributedString: titleAttributedString, backgroundColor: nil, maximumNumberOfLines: 1, truncationType: .end, constrainedSize: CGSize(width: titleRect.width, height: CGFloat.greatestFiniteMagnitude), cutout: nil)
