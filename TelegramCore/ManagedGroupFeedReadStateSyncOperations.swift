...
private func fetchReadStateNext(network: Network, groupId: PeerGroupId) -> Signal<GroupFeedReadState?, NoError> {
    /*%FEED return network.request(Api.functions.messages.getPeerDialogs(peers: [.inputPeerFeed(feedId: groupId.rawValue)]))
    |> retryRequest
...
private func fetchReadState(network: Network, groupId: PeerGroupId) -> Signal<GroupFeedReadState?, NoError> {
    /*%FEED return network.request(Api.functions.channels.getFeed(flags: 0, feedId: groupId.rawValue, offsetPosition: nil, addOffset: 0, limit: 1, maxPosition: nil, minPosition: nil, sourcesHash: 0, hash: 0))
        |> retryRequest
        |> map { result -> GroupFeedReadState? in
            switch result {
