        ...
        return { item, layoutConstants, _, constrainedSize in
            ...
            if let webpage = webpage {
                ...
                if let file = webpage.file {
                    if file.isVideo {
                        let (initialImageWidth, refineLayout) = contentImageLayout(account: item.account, media: file, corners: ImageCorners(radius: 4.0), automaticDownload: true, constrainedSize: CGSize(width: constrainedSize.width - insets.left - insets.right, height: constrainedSize.height))
                        initialWidth = initialImageWidth + insets.left + insets.right
                        refineContentImageLayout = refineLayout
                    } else {
                        let (_, refineLayout) = contentFileLayout(account: item.account, file: file, incoming: item.message.flags.contains(.Incoming), constrainedSize: CGSize(width: constrainedSize.width - insets.left - insets.right, height: constrainedSize.height))
                        refineContentFileLayout = refineLayout
                    }
                } else if let image = webpage.image {
                    if let type = webpage.type, ["photo"].contains(type) {
                        let (initialImageWidth, refineLayout) = contentImageLayout(account: item.account, media: image, corners: ImageCorners(radius: 4.0), automaticDownload: true, constrainedSize: CGSize(width: constrainedSize.width - insets.left - insets.right, height: constrainedSize.height))
                        initialWidth = initialImageWidth + insets.left + insets.right
                        refineContentImageLayout = refineLayout
            ...
            return (initialWidth, { constrainedSize in
                ...
                if refineContentImageLayout == nil && refineContentFileLayout == nil {
                    statusSizeAndApply = statusLayout(dateText: dateText, type: statusType, constrainedSize: textConstrainedSize)
                }
                
                let (textLayout, textApply) = textAsyncLayout(attributedString: textString, backgroundColor: nil, maximumNumberOfLines: 12, truncationType: .end, constrainedSize: textConstrainedSize, cutout: textCutout)
