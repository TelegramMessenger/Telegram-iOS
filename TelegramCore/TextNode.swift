...
final class TextNodeLayout: NSObject {
    ...
    private let truncationType: CTLineTruncationType
    private let backgroundColor: UIColor?
    private let constrainedSize: CGSize
    private let cutout: TextNodeCutout?
    let size: CGSize
    private let lines: [TextNodeLine]
    
    private init(attributedString: NSAttributedString?, maximumNumberOfLines: Int, truncationType: CTLineTruncationType, constrainedSize: CGSize, cutout: TextNodeCutout?, size: CGSize, lines: [TextNodeLine], backgroundColor: UIColor?) {
    ...
    @objc override class func draw(_ bounds: CGRect, withParameters parameters: NSObjectProtocol!, isCancelled: asdisplaynode_iscancelled_block_t, isRasterizing: Bool) {
        ...
        if let layout = parameters as? TextNodeLayout {
            ...
            for i in 0 ..< layout.lines.count {
                ...
                context.setTextPosition(x: line.frame.origin.x, y: line.frame.origin.y)
                CTLineDraw(line.line, context)
            }
            
            //CGContextRestoreGState(context)
            context.textMatrix = textMatrix
            context.setTextPosition(x: textPosition.x, y: textPosition.y)
        }
        
        context.setBlendMode(.normal)
    }
    
    class func asyncLayout(_ maybeNode: TextNode?) -> (attributedString: NSAttributedString?, backgroundColor: UIColor?, maximumNumberOfLines: Int, truncationType: CTLineTruncationType, constrainedSize: CGSize, cutout: TextNodeCutout?) -> (TextNodeLayout, () -> TextNode) {
