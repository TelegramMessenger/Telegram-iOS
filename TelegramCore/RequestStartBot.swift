...
public func requestStartBotInGroup(account: Account, botPeerId: PeerId, groupPeerId: PeerId, payload: String?) -> Signal<Void, NoError> {
    ...
    |> mapToSignal { botPeer, groupPeer -> Signal<Void, NoError> in
        if let botPeer = botPeer, let inputUser = apiInputUser(botPeer), let groupPeer = groupPeer, let inputGroup = apiInputPeer(groupPeer) {
            var randomId: Int64 = 0
            arc4random_buf(&randomId, 8)
            let r = account.network.request(Api.functions.messages.startBot(bot: inputUser, peer: inputGroup, randomId: randomId, startParam: payload ?? ""))
            |> mapToSignal { result -> Signal<Void, MTRpcError> in
                account.stateManager.addUpdates(result)
                return .complete()
            }
