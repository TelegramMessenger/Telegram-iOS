    ...
    override func asyncLayoutContent() -> (item: ListControllerGroupableItem, width: CGFloat) -> (CGSize, () -> Void) {
        ...
        return { item, width in
            if let item = item as? ListControllerDisclosureActionItem {
                let (labelLayout, labelApply) = layoutLabel(attributedString: NSAttributedString(string: item.title, font: titleFont, textColor: UIColor.black), backgroundColor: nil, maximumNumberOfLines: 1, truncationType: .end, constrainedSize: CGSize(width: width - 20, height: CGFloat.greatestFiniteMagnitude), cutout: nil)
