...
class ContactsVCardItem: ListViewItem {
    ...
    init(account: Account, peer: Peer, action: (Peer) -> Void) {
    ...
    func nodeConfiguredForWidth(async: (() -> Void) -> Void, width: CGFloat, previousItem: ListViewItem?, nextItem: ListViewItem?, completion: (ListViewItemNode, () -> Void) -> Void) {
        async {
            ...
            let (nodeLayout, nodeApply) = makeLayout(account: self.account, peer: self.peer, width: width, first: previousItem != nil, last: nextItem != nil)
        ...
        if let node = node as? ContactsVCardItemNode {
            Queue.mainQueue().async {
                ...
                async {
                    ...
                    let (nodeLayout, apply) = layout(account: self.account, peer: self.peer, width: width, first: first, last: last)
                    Queue.mainQueue().async {
...
class ContactsVCardItemNode: ListViewItemNode {
    ...
    func asyncLayout() -> (account: Account?, peer: Peer?, width: CGFloat, first: Bool, last: Bool) -> (ListViewItemNodeLayout, () -> Void) {
        ...
        return { [weak self] account, peer, width, first, last in
            ...
            let (titleLayout, titleApply) = makeTitleLayout(attributedString: titleAttributedString, backgroundColor: nil, maximumNumberOfLines: 1, truncationType: .end, constrainedSize: CGSize(width: max(0.0, width - leftInset - rightInset), height: CGFloat.infinity), cutout: nil)
            
            let (statusLayout, statusApply) = makeStatusLayout(attributedString: statusAttributedString, backgroundColor: nil, maximumNumberOfLines: 1, truncationType: .end, constrainedSize: CGSize(width: max(0.0, width - leftInset - rightInset), height: CGFloat.infinity), cutout: nil)
