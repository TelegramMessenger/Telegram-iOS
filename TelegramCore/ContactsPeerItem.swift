...
class ContactsPeerItem: ListViewItem {
    ...
    init(account: Account, peer: Peer, index: PeerNameIndex?, action: (Peer) -> Void) {
    ...
    func nodeConfiguredForWidth(async: (() -> Void) -> Void, width: CGFloat, previousItem: ListViewItem?, nextItem: ListViewItem?, completion: (ListViewItemNode, () -> Void) -> Void) {
        async {
            ...
            let (nodeLayout, nodeApply) = makeLayout(account: self.account, peer: self.peer, width: width, first: first, last: last)
