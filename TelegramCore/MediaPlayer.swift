...
private struct MediaPlayerLoadedState {
    ...
    private let mediaBuffers: MediaPlaybackBuffers
    private let controlTimebase: MediaPlayerControlTimebase
}
...
private final class MediaPlayerContext {
    ...
    private var status = Promise<MediaPlayerStatus>()
    ...
    private func seek(timestamp: Double, action: MediaPlayerPlaybackAction) {
        assert(self.queue.isCurrent())
    ...
    private func seekingCompleted(seekResult: MediaFrameSourceSeekResult) {
