...
fileprivate let parsers: [Int32 : (BufferReader) -> Any?] = {
    ...
    dict[-1237164374] = { return Api.PasswordKdfAlgo.parse_passwordKdfAlgoSHA256SHA256PBKDF2HMACSHA512iter100000($0) }
    dict[1753693093] = { return Api.account.Password.parse_password($0) }
...
struct messages {
    ...
    func descriptionFields() -> (String, [(String, Any)]) {
        switch self {
                ...
                case .messagesSlice(let count, let messages, let chats, let users):
                return ("messagesSlice", [("count", count), ("messages", messages), ("chats", chats), ("users", users)])
                case .channelMessages(let flags, let pts, let count, let messages, let chats, let users):
