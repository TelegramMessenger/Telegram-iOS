...
class ChatMessageForwardInfoNode: ASTransformLayerNode {
    ...
    class func asyncLayout(_ maybeNode: ChatMessageForwardInfoNode?) -> (incoming: Bool, peer: Peer, authorPeer: Peer?, constrainedSize: CGSize) -> (CGSize, () -> ChatMessageForwardInfoNode) {
        ...
        return { incoming, peer, authorPeer, constrainedSize in
            ...
            let completeString: NSString = "\(prefix)\(peerString)"
            ...
            let (textLayout, textApply) = textNodeLayout(attributedString: string, backgroundColor: nil, maximumNumberOfLines: 2, truncationType: .end, constrainedSize: constrainedSize, cutout: nil)
