...
@interface TGModernConversationAlphacodeAssociatedPanel () <UITableViewDelegate, UITableViewDataSource>
{
...
@implementation TGModernConversationAlphacodeAssociatedPanel
...
- (instancetype)initWithStyle:(TGModernConversationAssociatedInputPanelStyle)style
{
    ...
    if (self != nil)
    {
        ...
        if (iosMajorVersion() >= 7)
        {
            ...
            _tableView.separatorInset = UIEdgeInsetsMake(0.0f, 40.0f, 0.0f, 0.0f);
        }
...
- (CGFloat)preferredHeightAndOverlayHeight:(CGFloat *)overlayHeight
{
    ...
    NSInteger lastIndex = MIN([TGViewController isWidescreen] ? 4 : 3, (NSInteger)_codeList.count - 1);
    ...
    for (NSInteger i = 0; i < (NSInteger)_codeList.count; i++) {
        CGFloat rowHeight = [self tableView:_tableView heightForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
...
- (void)setAlphacodeListSignal:(SSignal *)alphacodeListSignal
{
    ...
    {
        __weak TGModernConversationAlphacodeAssociatedPanel *weakSelf = self;
        [_disposable setDisposable:[[alphacodeListSignal deliverOn:[SQueue mainQueue]] startWithNext:^(NSArray *userList)
        {
            __strong TGModernConversationAlphacodeAssociatedPanel *strongSelf = weakSelf;
            if (strongSelf != nil)
                [strongSelf setCodeList:userList];
        }]];
    }
}

- (void)setCodeList:(NSArray *)codeList
{
    bool wasEmpty = _codeList.count == 0;
    _codeList = codeList;
    
    if (iosMajorVersion() >= 7) {
        _tableView.separatorStyle = _codeList.count <= 1 ? UITableViewCellSeparatorStyleNone : UITableViewCellSeparatorStyleSingleLine;
    }
    ...
    _stripeView.hidden = _codeList.count == 0;
    _separatorView.hidden = _codeList.count == 0;
    _bottomView.hidden = _codeList.count == 0;
    
    [self scrollViewDidScroll:_tableView];
    
    if (_codeList.count != 0 && wasEmpty) {
        [self animateIn];
    } else {
        [self layoutSubviews];
    }
...
{
    TGAlphacodeEntry *entry = _codeList[indexPath.row];
    if (_alphacodeSelected) {
        _alphacodeSelected(entry);
    }
}
