...
public final class CachedChannelData: CachedPeerData {
    ...
    public let linkedMonoforumPeerId: LinkedDiscussionPeerId
    ...
    public init() {
        ...
        self.linkedMonoforumPeerId = .unknown
    ...
    public init(
        ...
        linkedMonoforumPeerId: LinkedDiscussionPeerId,
        ...
        self.linkedMonoforumPeerId = linkedMonoforumPeerId
        ...
        if case let .known(linkedMonoforumPeerIdValue) = linkedMonoforumPeerId {
    ...
    public func withUpdatedMinAvailableMessageId(_ minAvailableMessageId: MessageId?) -> CachedChannelData {
        return CachedChannelData(isNotAccessible: self.isNotAccessible, flags: self.flags, about: self.about, participantsSummary: self.participantsSummary, exportedInvitation: self.exportedInvitation, botInfos: self.botInfos, peerStatusSettings: self.peerStatusSettings, pinnedMessageId: self.pinnedMessageId, stickerPack: self.stickerPack, minAvailableMessageId: minAvailableMessageId, migrationReference: self.migrationReference, linkedDiscussionPeerId: self.linkedDiscussionPeerId, linkedMonoforumPeerId: self.linkedMonoforumPeerId, peerGeoLocation: self.peerGeoLocation, slowModeTimeout: self.slowModeTimeout, slowModeValidUntilTimestamp: self.slowModeValidUntilTimestamp, hasScheduledMessages: self.hasScheduledMessages, statsDatacenterId: self.statsDatacenterId, invitedBy: self.invitedBy, invitedOn: self.invitedOn, photo: self.photo, activeCall: self.activeCall, callJoinPeerId: self.callJoinPeerId, autoremoveTimeout: self.autoremoveTimeout, pendingSuggestions: self.pendingSuggestions, themeEmoticon: self.themeEmoticon, inviteRequestsPending: self.inviteRequestsPending, sendAsPeerId: self.sendAsPeerId, reactionSettings: self.reactionSettings, membersHidden: self.membersHidden, viewForumAsMessages: self.viewForumAsMessages, wallpaper: self.wallpaper, boostsToUnrestrict: self.boostsToUnrestrict, appliedBoosts: self.appliedBoosts, emojiPack: self.emojiPack, verification: self.verification, starGiftsCount: self.starGiftsCount)
    }
    ...
    public func withUpdatedLinkedMonoforumPeerId(_ linkedMonoforumPeerId: LinkedDiscussionPeerId) -> CachedChannelData {
    ...
    public init(decoder: PostboxDecoder) {
        ...
        if let linkedMonoforumPeerId = decoder.decodeOptionalInt64ForKey("monfrmi") {
            if linkedMonoforumPeerId == 0 {
                self.linkedMonoforumPeerId = .known(nil)
            } else {
                self.linkedMonoforumPeerId = .known(PeerId(linkedMonoforumPeerId))
            }
        } else {
            self.linkedMonoforumPeerId = .unknown
        }
        ...
        if case let .known(linkedMonoforumPeerIdValue) = self.linkedMonoforumPeerId {
            if let linkedMonoforumPeerIdValue = linkedMonoforumPeerIdValue {
                peerIds.insert(linkedMonoforumPeerIdValue)
            }
        }
    ...
    public func encode(_ encoder: PostboxEncoder) {
        ...
        switch self.linkedMonoforumPeerId {
        case .unknown:
            encoder.encodeNil(forKey: "monfrmi")
        case let .known(value):
            if let value = value {
                encoder.encodeInt64(value.toInt64(), forKey: "monfrmi")
            } else {
                encoder.encodeInt64(0, forKey: "monfrmi")
            }
        }
    ...
    public func isEqual(to: CachedPeerData) -> Bool {
        ...
        if other.linkedMonoforumPeerId != self.linkedMonoforumPeerId {
             return false
        }
