...
private enum AccountSelectionControllerEntry: ItemListNodeEntry {
    ...
    case account(Int32, PresentationDateTimeFormat, PresentationPersonNameOrder, EnginePeer, EquatableAccountContext)
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        case let .account(_, dateTimeFormat, nameDisplayOrder, peer, context):
            return ItemListPeerItem(presentationData: presentationData, dateTimeFormat: dateTimeFormat, nameDisplayOrder: nameDisplayOrder, context: context.context, peer: peer, nameStyle: .plain, presence: nil, text: .none, label: .disclosure(""), editing: ItemListPeerItemEditing(editable: false, editing: false, revealed: nil), switchValue: nil, enabled: true, selectable: true, sectionId: self.section, action: {
...
private func accountSelectionControllerEntries(presentationData: PresentationData, accounts: [(AccountContext, EnginePeer)]) -> [AccountSelectionControllerEntry] {
    ...
    for (index, value) in accounts.enumerated() {
        entries.append(.account(Int32(index), presentationData.dateTimeFormat, presentationData.nameDisplayOrder, value.1, EquatableAccountContext(context: value.0)))
    }
    
    return entries
}

public func accountSelectionController(context: AccountContext, accountSelected: @escaping (AccountContext) -> Void) -> ViewController {
    ...
    |> map { presentationData, accountsAndPeers -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        let listState = ItemListNodeState(presentationData: ItemListPresentationData(presentationData), entries: accountSelectionControllerEntries(presentationData: presentationData, accounts: accountsAndPeers.1.map({ ($0.0, $0.1) })), style: .blocks)
