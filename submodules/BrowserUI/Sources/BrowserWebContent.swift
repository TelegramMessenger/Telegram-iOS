        ...
        let js = """
            (function() {
                ...
                for (var i = 0; i < nodeList.length; i++)
                {
                    ...
                    const relAttrValue = node.getAttribute('rel');

                    if (typeof relAttrValue === 'string' && relAttrValue && (relAttrValue === 'icon' || relAttrValue === 'shortcut icon' || relAttrValue.startsWith('apple-touch-icon')))
                    {
                        favicons.push({
