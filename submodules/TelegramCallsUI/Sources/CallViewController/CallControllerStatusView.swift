...
final class CallControllerStatusView: UIView {
    private let titleNode: TextNode
    private let statusContainerNode: UIView
    private let statusNode: TextNode
    private let statusMeasureNode: TextNode
    private let receptionNode: CallControllerReceptionView
    ...
    var status: CallControllerStatusValue = .text(string: "", displayLogo: false) {
        didSet {
            if self.status != oldValue {
                ...
                if let snapshotView = self.statusContainerNode.snapshotView(afterScreenUpdates: false) {
                    snapshotView.frame = self.statusContainerNode.frame
                    self.insertSubview(snapshotView, belowSubview: self.statusContainerNode)
            ...
            if self.reception != oldValue {
                if let reception = self.reception {
                    ...
                    if oldValue == nil {
                        ...
                        transition.updateAlpha(view: self.receptionNode, alpha: 1.0)
                    }
                } else if self.reception == nil, oldValue != nil {
                    let transition = ContainedViewLayoutTransition.animated(duration: 0.3, curve: .spring)
                    transition.updateAlpha(view: self.receptionNode, alpha: 0.0)
                }
    ...
    init() {
        ...
        self.addSubview(self.statusContainerNode)
        self.statusContainerNode.addSubnode(self.statusNode)
        self.statusContainerNode.addSubview(self.receptionNode)
    ...
    func setVisible(_ visible: Bool, transition: ContainedViewLayoutTransition) {
        ...
        transition.updateAlpha(view: self.statusContainerNode, alpha: alpha)
    }
    ...
    func updateLayout(constrainedWidth: CGFloat, transition: ContainedViewLayoutTransition) -> CGFloat {
        ...
        self.statusNode.frame = CGRect(origin: CGPoint(x: floor((constrainedWidth - statusMeasureLayout.size.width) / 2.0) + statusOffset, y: -5.0), size: statusLayout.size)
        self.receptionNode.frame = CGRect(origin: CGPoint(x: self.statusNode.frame.minX - receptionNodeSize.width, y: 4.0), size: receptionNodeSize)
        ...
        if let image = self.logoNode.image, let firstLineRect = statusMeasureLayout.linesRects().first {
            ...
            self.logoNode.frame = CGRect(origin: CGPoint(x: self.statusNode.frame.minX + firstLineOffset - image.size.width - 7.0, y: 0.0), size: image.size)
        }
...
final class CallControllerReceptionView : UIView {
