...
final class VoiceChatTileItemView: UIView {
    private let context: AccountContext
  
    let contextSourceNode: ContextExtractedContentContainingView
    private let containerNode: ContextControllerSourceView
    ...
    let fadeNode: UIView
    private var shimmerNode: VoiceChatTileShimmeringView?
    private let titleNode: ImmediateTextNode
    private var iconNode: ASImageNode?
    private var animationNode: VoiceChatMicrophoneNode?
    var highlightNode: VoiceChatTileHighlightView
    ...
    init(context: AccountContext) {
        ...
        self.contextSourceNode = ContextExtractedContentContainingView()
        self.containerNode = ContextControllerSourceView()
        
        self.contentNode = UIView()
        self.contentNode.clipsToBounds = true
        self.contentNode.layer.cornerRadius = backgroundCornerRadius
        ...
        self.highlightNode = VoiceChatTileHighlightView()
        ...
        self.containerNode.addSubview(self.contextSourceNode)
        self.containerNode.targetViewForActivationProgress = self.contextSourceNode.contentView
        self.addSubview(self.containerNode)
        
        self.contextSourceNode.contentView.addSubview(self.contentNode)
        self.contentNode.addSubnode(self.backgroundNode)
        self.contentNode.addSubnode(self.videoContainerNode)
        self.contentNode.addSubview(self.fadeNode)
        ...
        self.contentNode.addSubview(self.highlightNode)
...
//            contextAction(strongSelf.contextSourceNode, gesture)
    ...
    private func updateIsExtracted(_ isExtracted: Bool, transition: ContainedViewLayoutTransition) {
        ...
        if isExtracted {
            ...
            self.contextSourceNode.contentView.addSubnode(profileNode)
...
//            profileNode.animateIn(from: self, targetRect: extractedRect, transition: transition)
            ...
            self.contextSourceNode.contentView.customHitTest = { [weak self] point in
...
//            })
            ...
            self.contextSourceNode.contentView.customHitTest = nil
        }
    }
    ...
    func update(size: CGSize, availableWidth: CGFloat, item: VoiceChatTileItem, transition: ContainedViewLayoutTransition) {
        ...
        if !item.videoReady || item.isOwnScreencast {
            let shimmerNode: VoiceChatTileShimmeringView
            let shimmerTransition: ContainedViewLayoutTransition
            if let current = self.shimmerNode {
                shimmerNode = current
                shimmerTransition = transition
            } else {
                shimmerNode = VoiceChatTileShimmeringView(account: item.account, peer: item.peer)
                self.contentNode.insertSubview(shimmerNode, aboveSubview: self.fadeNode)
            ...
            shimmerTransition.updateFrame(view: shimmerNode, frame: CGRect(origin: CGPoint(), size: size))
            ...
            shimmerNode.layer.animateAlpha(from: 1.0, to: 0.0, duration: 0.3, removeOnCompletion: false, completion: { [weak shimmerNode] _ in
                shimmerNode?.removeFromSuperview()
            })
        }
        ...
        if self.item != item {
            ...
            transition.updateAlpha(view: self.highlightNode, alpha: item.speaking ? 1.0 : 0.0)
        ...
        self.contextSourceNode.contentView.frame = bounds
        
        transition.updateFrame(view: self.contentNode, frame: bounds)
        ...
        transition.updateFrame(view: self.highlightNode, frame: bounds)
        ...
        transition.updateFrame(view: self.fadeNode, frame: CGRect(x: 0.0, y: size.height - fadeHeight, width: size.width, height: fadeHeight))
...
class VoiceChatTileHighlightView: UIView {
    enum Gradient {
        case speaking
        case active
        case mutedForYou
    ...
    init() {
        ...
        let customMaskView = UIView()
        customMaskView.layer.addSublayer(self.maskLayer)
        self.customMaskView = customMaskView
    ...
    func updateLayout(size: CGSize, transition: ContainedViewLayoutTransition) {
        let bounds = CGRect(origin: CGPoint(), size: size)
        if let maskView = self.customMaskView {
...
final class ShimmerEffectForegroundView: UIView {
...
private class VoiceChatTileShimmeringView: UIView {
    private let backgroundNode: ImageNode
    private let effectNode: ShimmerEffectForegroundView
    ...
    public init(account: Account, peer: Peer) {
        ...
        self.effectNode = ShimmerEffectForegroundView(size: 240.0)
        ...
        self.addSubview(self.effectNode)
    ...
    public func update(shimmeringColor: UIColor, shimmering: Bool, size: CGSize, transition: ContainedViewLayoutTransition) {
        ...
        transition.updateAlpha(view: self.effectNode, alpha: shimmering ? 1.0 : 0.0)
        ...
        transition.updateFrame(view: self.effectNode, frame: bounds)
