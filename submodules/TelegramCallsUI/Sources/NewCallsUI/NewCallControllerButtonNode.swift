...
final class NewCallControllerButtonsNode: ASDisplayNode {
    private var buttonNodes: [ButtonDescription.Key: NewCallControllerButtonItemNode] = [:]
    ...
    private func updateButtonsLayout(strings: PresentationStrings, mode: CallControllerButtonsMode, width: CGFloat, bottomInset: CGFloat, animated: Bool) -> CGFloat {
        ...
        let maxSmallButtonSpacing: CGFloat = 55.0
        let smallButtonSize: CGFloat = 55.0
        let topBottomSpacing: CGFloat = 30
        
        let maxLargeButtonSpacing: CGFloat = 115.0
        let largeButtonSize: CGFloat = 55.0
        ...
        case .incomingRinging, .outgoingRinging:
            ...
            height = topBottomSpacing + largeButtonSize + max(bottomInset + 20, 46.0)
        case .active:
            if videoState.hasVideo {
                let isCameraActive: Bool
                let isScreencastActive: Bool
                ...
                height = smallButtonSize + max(bottomInset + 20, 46.0)
            } else {
                var topButtons: [ButtonDescription] = []
                ...
                height = largeButtonSize + topBottomSpacing + max(bottomInset + 20, 46.0)
            }
        }
        ...
        for button in buttons {
            ...
            let buttonNode: NewCallControllerButtonItemNode
            if let current = self.buttonNodes[button.button.key] {
                buttonNode = current
            } else {
                buttonNode = NewCallControllerButtonItemNode()
            ...
            let buttonContent: NewCallControllerButtonItemNode.Content
            ...
            case .accept:
                buttonContent = NewCallControllerButtonItemNode.Content(
            ...
            case let .soundOutput(value):
                let image: NewCallControllerButtonItemNode.Content.Image
                var isFilled = false
    ...
    @objc func buttonPressed(_ button: NewCallControllerButtonItemNode) {
