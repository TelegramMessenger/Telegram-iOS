...
final class TonTransactionsScreenComponent: Component {
    ...
    static func ==(lhs: TonTransactionsScreenComponent, rhs: TonTransactionsScreenComponent) -> Bool {
        if lhs.context !== rhs.context {
            return false
        }
        if lhs.tonContext !== rhs.tonContext {
            return false
        }
        return true
    }
    ...
    class View: UIView, UIScrollViewDelegate {
        ...
        private var component: TonTransactionsScreenComponent?
        ...
        func update(component: TonTransactionsScreenComponent, availableSize: CGSize, state: EmptyComponentState, environment: Environment<ViewControllerComponentContainer.Environment>, transition: ComponentTransition) -> CGSize {
            ...
            if self.stateDisposable == nil {
                self.stateDisposable = (component.tonContext.state
            ...
            let starSize = self.starView.update(
                transition: .immediate,
                component: AnyComponent(PremiumDiamondComponent()),
                environment: {},
            ...
            let titleSize = self.titleView.update(
                ...
                component: AnyComponent(
                    MultilineTextComponent(
                        text: .plain(NSAttributedString(string: "TON", font: Font.bold(28.0), textColor: environment.theme.list.itemPrimaryTextColor)),
            ...
            let topBalanceIconSize = self.topBalanceIconView.update(
                transition: .immediate,
                component: AnyComponent(BundleIconComponent(name: "Ads/TonAbout", tintColor: nil)),
                environment: {},
                containerSize: availableSize
            )
            ...
            let descriptionSize = self.descriptionView.update(
                ...
                component: AnyComponent(
                    BalancedTextComponent(
                        text: .plain(NSAttributedString(string: "Offer TON to submit post suggestions to channels on Telegram.", font: Font.regular(15.0), textColor: environment.theme.list.itemPrimaryTextColor)),
                        horizontalAlignment: .center,
            ...
            let withdrawAvailable = "".isEmpty //(self.revenueState?.balances.overallRevenue.value ?? 0) > 0
            ...
            if !initialTransactions.isEmpty {
                ...
                if let current = self.allTransactionsContext {
                    ...
                    allTransactionsContext = component.context.engine.payments.peerStarsTransactionsContext(subject: .starsContext(component.tonContext), mode: .all)
                ...
                if let current = self.incomingTransactionsContext {
                    ...
                    incomingTransactionsContext = component.context.engine.payments.peerStarsTransactionsContext(subject: .starsContext(component.tonContext), mode: .incoming)
                ...
                if let current = self.outgoingTransactionsContext {
                    ...
                    outgoingTransactionsContext = component.context.engine.payments.peerStarsTransactionsContext(subject: .starsContext(component.tonContext), mode: .outgoing)
...
public final class TonTransactionsScreen: ViewControllerComponentContainer {
    private let context: AccountContext
    private let tonContext: StarsContext
    ...
    public init(context: AccountContext, tonContext: StarsContext, forceDark: Bool = false) {
        self.context = context
        ...
        super.init(context: context, component: TonTransactionsScreenComponent(
            context: self.context,
            starsContext: self.tonContext,
        ...
        buyImpl = { [weak self] in
            ...
            |> deliverOnMainQueue).start(next: { [weak self] options in
                ...
                let controller = context.sharedContext.makeStarsPurchaseScreen(context: context, starsContext: tonContext, options: options, purpose: .generic, completion: { [weak self] stars in
                    guard let self else {
                        return
                    }
                    self.tonContext.add(balance: StarsAmount(value: stars, nanos: 0))
...
//            })
        ...
        self.tonContext.load(force: false)
        
        self.scrollToTop = { [weak self] in
            guard let self else {
                return
            }
            if let componentView = self.node.hostView.componentView as? TonTransactionsScreenComponent.View {
