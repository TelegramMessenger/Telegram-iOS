...
TgJsepTransportController::TgJsepTransportController(
...
TgJsepTransportController::~TgJsepTransportController() {
  ...
  network_thread_->Invoke<void>(
      RTC_FROM_HERE,
      rtc::Bind(&TgJsepTransportController::DestroyAllJsepTransports_n, this));
}

RTCError TgJsepTransportController::SetLocalDescription(
    SdpType type,
...
RTCError TgJsepTransportController::SetRemoteDescription(
    SdpType type,
    const cricket::SessionDescription* description) {
  if (!network_thread_->IsCurrent()) {
    return network_thread_->Invoke<RTCError>(
...
RtpTransportInternal* TgJsepTransportController::GetRtpTransport(
...
MediaTransportConfig TgJsepTransportController::GetMediaTransportConfig(
    const std::string& mid) const {
  auto jsep_transport = GetJsepTransportForMid(mid);
  if (!jsep_transport) {
...
DataChannelTransportInterface* TgJsepTransportController::GetDataChannelTransport(
    const std::string& mid) const {
  auto jsep_transport = GetJsepTransportForMid(mid);
  if (!jsep_transport) {
    return nullptr;
...
cricket::DtlsTransportInternal* TgJsepTransportController::GetDtlsTransport(
...
TgJsepTransportController::GetRtcpDtlsTransport(const std::string& mid) const {
  auto jsep_transport = GetJsepTransportForMid(mid);
  if (!jsep_transport) {
    return nullptr;
  }
...
TgJsepTransportController::LookupDtlsTransportByMid(const std::string& mid) {
  auto jsep_transport = GetJsepTransportForMid(mid);
  if (!jsep_transport) {
    return nullptr;
  }
...
rtc::scoped_refptr<SctpTransport> TgJsepTransportController::GetSctpTransport(
    const std::string& mid) const {
  auto jsep_transport = GetJsepTransportForMid(mid);
  if (!jsep_transport) {
    return nullptr;
...
void TgJsepTransportController::SetIceConfig(const cricket::IceConfig& config) {
  if (!network_thread_->IsCurrent()) {
...
void TgJsepTransportController::SetNeedsIceRestartFlag() {
  for (auto& kv : jsep_transports_by_name_) {
    kv.second->SetNeedsIceRestartFlag();
  }
}

bool TgJsepTransportController::NeedsIceRestart(
    const std::string& transport_name) const {
  const cricket::TgJsepTransport* transport =
...
absl::optional<rtc::SSLRole> TgJsepTransportController::GetDtlsRole(
    const std::string& mid) const {
  if (!network_thread_->IsCurrent()) {
  ...
  const cricket::TgJsepTransport* t = GetJsepTransportForMid(mid);
  if (!t) {
...
bool TgJsepTransportController::SetLocalCertificate(
  ...
  // Set certificate for TgJsepTransport, which verifies it matches the
...
TgJsepTransportController::GetLocalCertificate(
    const std::string& transport_name) const {
  if (!network_thread_->IsCurrent()) {
  ...
  const cricket::TgJsepTransport* t = GetJsepTransportByName(transport_name);
  if (!t) {
    return nullptr;
  }
...
TgJsepTransportController::GetRemoteSSLCertChain(
    const std::string& transport_name) const {
  if (!network_thread_->IsCurrent()) {
...
void TgJsepTransportController::MaybeStartGathering() {
  if (!network_thread_->IsCurrent()) {
...
RTCError TgJsepTransportController::AddRemoteCandidates(
  ...
  auto jsep_transport = GetJsepTransportByName(transport_name);
  if (!jsep_transport) {
    RTC_LOG(LS_WARNING) << "Not adding candidate because the TgJsepTransport "
...
RTCError TgJsepTransportController::RemoveRemoteCandidates(
    const cricket::Candidates& candidates) {
  if (!network_thread_->IsCurrent()) {
    return network_thread_->Invoke<RTCError>(
  ...
  for (const auto& kv : candidates_by_transport_name) {
    ...
    if (!jsep_transport) {
      RTC_LOG(LS_WARNING)
          << "Not removing candidate because the TgJsepTransport doesn't exist.";
      continue;
    }
    for (const cricket::Candidate& candidate : candidates) {
...
bool TgJsepTransportController::GetStats(const std::string& transport_name,
  ...
  cricket::TgJsepTransport* transport = GetJsepTransportByName(transport_name);
  if (!transport) {
    return false;
  }
...
void TgJsepTransportController::SetActiveResetSrtpParams(
...
void TgJsepTransportController::SetMediaTransportSettings(
...
void TgJsepTransportController::RollbackTransportForMids(
...
TgJsepTransportController::CreateIceTransport(const std::string& transport_name,
                                            bool rtcp) {
...
TgJsepTransportController::CreateDtlsTransport(
  ...
  dtls->SignalWritableState.connect(
      this, &TgJsepTransportController::OnTransportWritableState_n);
  dtls->SignalReceivingState.connect(
      this, &TgJsepTransportController::OnTransportReceivingState_n);
  dtls->SignalDtlsHandshakeError.connect(
      this, &TgJsepTransportController::OnDtlsHandshakeError);
  dtls->ice_transport()->SignalGatheringState.connect(
      this, &TgJsepTransportController::OnTransportGatheringState_n);
  dtls->ice_transport()->SignalCandidateGathered.connect(
      this, &TgJsepTransportController::OnTransportCandidateGathered_n);
  dtls->ice_transport()->SignalCandidateError.connect(
      this, &TgJsepTransportController::OnTransportCandidateError_n);
  dtls->ice_transport()->SignalCandidatesRemoved.connect(
      this, &TgJsepTransportController::OnTransportCandidatesRemoved_n);
  dtls->ice_transport()->SignalRoleConflict.connect(
      this, &TgJsepTransportController::OnTransportRoleConflict_n);
  dtls->ice_transport()->SignalStateChanged.connect(
      this, &TgJsepTransportController::OnTransportStateChanged_n);
  dtls->ice_transport()->SignalIceTransportStateChanged.connect(
      this, &TgJsepTransportController::OnTransportStateChanged_n);
  dtls->ice_transport()->SignalCandidatePairChanged.connect(
      this, &TgJsepTransportController::OnTransportCandidatePairChanged_n);
  return dtls;
}

std::unique_ptr<webrtc::TgRtpTransport>
TgJsepTransportController::CreateUnencryptedRtpTransport(
    const std::string& transport_name,
    rtc::PacketTransportInternal* rtp_packet_transport,
    rtc::PacketTransportInternal* rtcp_packet_transport) {
  RTC_DCHECK(network_thread_->IsCurrent());
  auto unencrypted_rtp_transport =
      std::make_unique<TgRtpTransport>(rtcp_packet_transport == nullptr);
...
TgJsepTransportController::CreateSdesTransport(
    const std::string& transport_name,
...
TgJsepTransportController::CreateDtlsSrtpTransport(
    const std::string& transport_name,
    cricket::DtlsTransportInternal* rtp_dtls_transport,
    cricket::DtlsTransportInternal* rtcp_dtls_transport) {
  RTC_DCHECK(network_thread_->IsCurrent());
  ...
  dtls_srtp_transport->SignalDtlsStateChange.connect(
      this, &TgJsepTransportController::UpdateAggregateStates_n);
  return dtls_srtp_transport;
}

std::vector<cricket::DtlsTransportInternal*>
TgJsepTransportController::GetDtlsTransports() {
...
RTCError TgJsepTransportController::ApplyDescription_n(
    bool local,
    SdpType type,
    const cricket::SessionDescription* description) {
  RTC_DCHECK(network_thread_->IsCurrent());
  ...
  for (size_t i = 0; i < description->contents().size(); ++i) {
    ...
    cricket::TgJsepTransportDescription jsep_description =
...
RTCError TgJsepTransportController::ValidateAndMaybeUpdateBundleGroup(
    bool local,
    SdpType type,
    const cricket::SessionDescription* description) {
  RTC_DCHECK(description);
...
RTCError TgJsepTransportController::ValidateContent(
    const cricket::ContentInfo& content_info) {
...
bool TgJsepTransportController::HandleBundledContent(
    const cricket::ContentInfo& content_info) {
...
bool TgJsepTransportController::SetTransportForMid(
    const std::string& mid,
    cricket::TgJsepTransport* jsep_transport) {
  RTC_DCHECK(jsep_transport);
...
void TgJsepTransportController::RemoveTransportForMid(const std::string& mid) {
...
cricket::TgJsepTransportDescription
TgJsepTransportController::CreateJsepTransportDescription(
    const cricket::ContentInfo& content_info,
    const cricket::TransportInfo& transport_info,
  ...
  return cricket::TgJsepTransportDescription(
...
bool TgJsepTransportController::ShouldUpdateBundleGroup(
    SdpType type,
    const cricket::SessionDescription* description) {
  if (config_.bundle_policy ==
      PeerConnectionInterface::kBundlePolicyMaxBundle) {
...
std::vector<int> TgJsepTransportController::GetEncryptedHeaderExtensionIds(
    const cricket::ContentInfo& content_info) {
  const cricket::MediaContentDescription* content_desc =
      content_info.media_description();
...
std::vector<int>
TgJsepTransportController::MergeEncryptedHeaderExtensionIdsForBundle(
    const cricket::SessionDescription* description) {
  RTC_DCHECK(description);
...
RTCError TgJsepTransportController::GetAltProtocolsForBundle(
    const cricket::SessionDescription* description,
    absl::optional<std::string>* media_alt_protocol,
    absl::optional<std::string>* data_alt_protocol) {
  RTC_DCHECK(description);
...
int TgJsepTransportController::GetRtpAbsSendTimeHeaderExtensionId(
    const cricket::ContentInfo& content_info) {
  if (!config_.enable_external_auth) {
    return -1;
  }
...
const cricket::TgJsepTransport* TgJsepTransportController::GetJsepTransportForMid(
    const std::string& mid) const {
  auto it = mid_to_transport_.find(mid);
  return it == mid_to_transport_.end() ? nullptr : it->second;
}

cricket::TgJsepTransport* TgJsepTransportController::GetJsepTransportForMid(
    const std::string& mid) {
  auto it = mid_to_transport_.find(mid);
  return it == mid_to_transport_.end() ? nullptr : it->second;
}

const cricket::TgJsepTransport* TgJsepTransportController::GetJsepTransportByName(
    const std::string& transport_name) const {
  auto it = jsep_transports_by_name_.find(transport_name);
  return (it == jsep_transports_by_name_.end()) ? nullptr : it->second.get();
}

cricket::TgJsepTransport* TgJsepTransportController::GetJsepTransportByName(
    const std::string& transport_name) {
  auto it = jsep_transports_by_name_.find(transport_name);
  return (it == jsep_transports_by_name_.end()) ? nullptr : it->second.get();
}
...
TgJsepTransportController::MaybeCreateDatagramTransport(
    const cricket::ContentInfo& content_info,
    const cricket::SessionDescription& description,
    bool local) {
  ...
  RTC_DCHECK(!local);
  // When bundle is enabled, two TgJsepTransports are created, and then
...
RTCError TgJsepTransportController::MaybeCreateJsepTransport(
    bool local,
    const cricket::ContentInfo& content_info,
    const cricket::SessionDescription& description) {
  RTC_DCHECK(network_thread_->IsCurrent());
  cricket::TgJsepTransport* transport = GetJsepTransportByName(content_info.name);
  if (transport) {
    return RTCError::OK();
  }
  ...
  jsep_transport->rtp_transport()->SignalRtcpPacketReceived.connect(
      this, &TgJsepTransportController::OnRtcpPacketReceived_n);

  jsep_transport->SignalRtcpMuxActive.connect(
      this, &TgJsepTransportController::UpdateAggregateStates_n);
  jsep_transport->SignalDataChannelTransportNegotiated.connect(
      this, &TgJsepTransportController::OnDataChannelTransportNegotiated_n);
...
void TgJsepTransportController::MaybeDestroyJsepTransport(
    const std::string& mid) {
...
void TgJsepTransportController::DestroyAllJsepTransports_n() {
  RTC_DCHECK(network_thread_->IsCurrent());
...
void TgJsepTransportController::SetIceRole_n(cricket::IceRole ice_role) {
  RTC_DCHECK(network_thread_->IsCurrent());
...
cricket::IceRole TgJsepTransportController::DetermineIceRole(
  ...
  if (local) {
    ...
    // SetLocalDescription in TgJsepTransportController.
...
void TgJsepTransportController::OnTransportWritableState_n(
...
void TgJsepTransportController::OnTransportGatheringState_n(
    cricket::IceTransportInternal* transport) {
  RTC_DCHECK(network_thread_->IsCurrent());
  UpdateAggregateStates_n();
}

void TgJsepTransportController::OnTransportCandidateGathered_n(
    cricket::IceTransportInternal* transport,
...
void TgJsepTransportController::OnTransportCandidateError_n(
    cricket::IceTransportInternal* transport,
...
void TgJsepTransportController::OnTransportCandidatesRemoved_n(
    cricket::IceTransportInternal* transport,
    const cricket::Candidates& candidates) {
  invoker_.AsyncInvoke<void>(
...
void TgJsepTransportController::OnTransportCandidatePairChanged_n(
...
void TgJsepTransportController::OnTransportRoleConflict_n(
    cricket::IceTransportInternal* transport) {
  RTC_DCHECK(network_thread_->IsCurrent());
...
void TgJsepTransportController::OnTransportStateChanged_n(
    cricket::IceTransportInternal* transport) {
  RTC_DCHECK(network_thread_->IsCurrent());
...
void TgJsepTransportController::OnDataChannelTransportNegotiated_n(
    cricket::TgJsepTransport* transport,
...
void TgJsepTransportController::UpdateAggregateStates_n() {
  RTC_DCHECK(network_thread_->IsCurrent());
  ...
  int total_ice = (int)dtls_transports.size();
...
void TgJsepTransportController::OnRtcpPacketReceived_n(
...
void TgJsepTransportController::OnDtlsHandshakeError(
    rtc::SSLHandshakeError error) {
  SignalDtlsHandshakeError(error);
}

absl::optional<cricket::OpaqueTransportParameters>
TgJsepTransportController::GetTransportParameters(const std::string& mid) {
  if (!(config_.use_datagram_transport ||
        config_.use_datagram_transport_for_data_channels)) {
    return absl::nullopt;
  }

  cricket::TgJsepTransport* transport = GetJsepTransportForMid(mid);
  if (transport) {
  ...
  RTC_DCHECK(!local_desc_ && !remote_desc_)
      << "TgJsepTransport should exist for every mid once any description is set";
