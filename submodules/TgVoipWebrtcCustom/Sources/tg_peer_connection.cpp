...
class TgPeerConnection::ImplicitCreateSessionDescriptionObserver
    : public CreateSessionDescriptionObserver {
 public:
  ImplicitCreateSessionDescriptionObserver(
      rtc::WeakPtr<TgPeerConnection> pc,
  ...
  void OnSuccess(SessionDescriptionInterface* desc_ptr) override {
    ...
    // invoked asynchronously in a post to TgPeerConnection::OnMessage().
    ...
    // completed here (rather than in TgPeerConnection::OnMessage()). This ensures
 ...
 private:
  bool was_called_ = false;
  rtc::WeakPtr<TgPeerConnection> pc_;
  rtc::scoped_refptr<SetSessionDescriptionObserver>
      set_local_description_observer_;
  std::function<void()> operation_complete_callback_;
};

class TgPeerConnection::LocalIceCredentialsToReplace {
 public:
...
class TgPeerConnection::SetRemoteDescriptionObserverAdapter
 ...
 private:
  rtc::scoped_refptr<TgPeerConnection> pc_;
...
void TgPeerConnection::TransceiverStableState::SetMSectionIfUnset(
...
void TgPeerConnection::TransceiverStableState::SetRemoteStreamIdsIfUnset(
...
TgPeerConnection::TgPeerConnection(TgPeerConnectionFactory* factory,
...
TgPeerConnection::~TgPeerConnection() {
  TRACE_EVENT0("webrtc", "TgPeerConnection::~TgPeerConnection");
  ...
  for (auto& msg : list) {
    if (msg.message_id == MSG_CREATE_SESSIONDESCRIPTION_FAILED) {
      ...
      // observers are invoked even if the TgPeerConnection is destroyed early.
      OnMessage(&msg);
    } else {
...
void TgPeerConnection::DestroyAllChannels() {
...
bool TgPeerConnection::Initialize(
    const PeerConnectionInterface::RTCConfiguration& configuration,
    PeerConnectionDependencies dependencies) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  TRACE_EVENT0("webrtc", "TgPeerConnection::Initialize");
  ...
  if (!dependencies.allocator) {
    RTC_LOG(LS_ERROR)
        << "TgPeerConnection initialized without a PortAllocator? "
  ...
  if (!dependencies.observer) {
    ...
    RTC_LOG(LS_ERROR) << "TgPeerConnection initialized without a "
  ...
  const auto pa_result =
      network_thread()->Invoke<InitializePortAllocatorResult>(
          RTC_FROM_HERE,
          rtc::Bind(&TgPeerConnection::InitializePortAllocator_n, this,
  ...
  TgJsepTransportController::Config config;
  ...
  // since the  TgJsepTransportController instance is owned by this PeerConnection
  ...
  transport_controller_.reset(new TgJsepTransportController(
      signaling_thread(), network_thread(), port_allocator_.get(),
      async_resolver_factory_.get(), config));
  transport_controller_->SignalIceConnectionState.connect(
      this, &TgPeerConnection::OnTransportControllerConnectionState);
  transport_controller_->SignalStandardizedIceConnectionState.connect(
      this, &TgPeerConnection::SetStandardizedIceConnectionState);
  transport_controller_->SignalConnectionState.connect(
      this, &TgPeerConnection::SetConnectionState);
  transport_controller_->SignalIceGatheringState.connect(
      this, &TgPeerConnection::OnTransportControllerGatheringState);
  transport_controller_->SignalIceCandidatesGathered.connect(
      this, &TgPeerConnection::OnTransportControllerCandidatesGathered);
  transport_controller_->SignalIceCandidateError.connect(
      this, &TgPeerConnection::OnTransportControllerCandidateError);
  transport_controller_->SignalIceCandidatesRemoved.connect(
      this, &TgPeerConnection::OnTransportControllerCandidatesRemoved);
  transport_controller_->SignalDtlsHandshakeError.connect(
      this, &TgPeerConnection::OnTransportControllerDtlsHandshakeError);
  ...
  webrtc_session_desc_factory_->SignalCertificateReady.connect(
      this, &TgPeerConnection::OnCertificateReady);
...
rtc::scoped_refptr<StreamCollectionInterface> TgPeerConnection::local_streams() {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
rtc::scoped_refptr<StreamCollectionInterface> TgPeerConnection::remote_streams() {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
bool TgPeerConnection::AddStream(MediaStreamInterface* local_stream) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_CHECK(!IsUnifiedPlan()) << "AddStream is not available with Unified Plan "
                                 "SdpSemantics. Please use AddTrack instead.";
  TRACE_EVENT0("webrtc", "TgPeerConnection::AddStream");
  if (IsClosed()) {
    return false;
  }
  ...
  observer->SignalAudioTrackAdded.connect(this,
                                          &TgPeerConnection::OnAudioTrackAdded);
  observer->SignalAudioTrackRemoved.connect(
      this, &TgPeerConnection::OnAudioTrackRemoved);
  observer->SignalVideoTrackAdded.connect(this,
                                          &TgPeerConnection::OnVideoTrackAdded);
  observer->SignalVideoTrackRemoved.connect(
      this, &TgPeerConnection::OnVideoTrackRemoved);
...
void TgPeerConnection::RemoveStream(MediaStreamInterface* local_stream) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_CHECK(!IsUnifiedPlan()) << "RemoveStream is not available with Unified "
                                 "Plan SdpSemantics. Please use RemoveTrack "
                                 "instead.";
  TRACE_EVENT0("webrtc", "TgPeerConnection::RemoveStream");
  if (!IsClosed()) {
    for (const auto& track : local_stream->GetAudioTracks()) {
...
RTCErrorOr<rtc::scoped_refptr<RtpSenderInterface>> TgPeerConnection::AddTrack(
    rtc::scoped_refptr<MediaStreamTrackInterface> track,
    const std::vector<std::string>& stream_ids) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  TRACE_EVENT0("webrtc", "TgPeerConnection::AddTrack");
  if (!track) {
...
TgPeerConnection::FindFirstTransceiverForAddedTrack(
    rtc::scoped_refptr<MediaStreamTrackInterface> track) {
  RTC_DCHECK(track);
...
bool TgPeerConnection::RemoveTrack(RtpSenderInterface* sender) {
  TRACE_EVENT0("webrtc", "TgPeerConnection::RemoveTrack");
  return RemoveTrackNew(sender).ok();
}

RTCError TgPeerConnection::RemoveTrackNew(
...
rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
TgPeerConnection::FindTransceiverBySender(
    rtc::scoped_refptr<RtpSenderInterface> sender) {
  for (auto transceiver : transceivers_) {
...
TgPeerConnection::AddTransceiver(
    rtc::scoped_refptr<MediaStreamTrackInterface> track) {
...
RTCErrorOr<rtc::scoped_refptr<RtpTransceiverInterface>>
TgPeerConnection::AddTransceiver(cricket::MediaType media_type) {
...
TgPeerConnection::AddTransceiver(
  ...
  RTC_HISTOGRAM_COUNTS_LINEAR(kSimulcastNumberOfEncodings,
                              (int)init.send_encodings.size(), 0, 7, 8);
...
TgPeerConnection::CreateReceiver(cricket::MediaType media_type,
...
TgPeerConnection::CreateAndAddTransceiver(
  ...
  transceiver->internal()->SignalNegotiationNeeded.connect(
      this, &TgPeerConnection::OnNegotiationNeeded);
  return transceiver;
}

void TgPeerConnection::OnNegotiationNeeded() {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_DCHECK(!IsClosed());
  UpdateNegotiationNeeded();
}

rtc::scoped_refptr<RtpSenderInterface> TgPeerConnection::CreateSender(
  ...
  RTC_CHECK(!IsUnifiedPlan()) << "CreateSender is not available with Unified "
                                 ...
                                 "instead.";
  TRACE_EVENT0("webrtc", "TgPeerConnection::CreateSender");
  if (IsClosed()) {
    return nullptr;
  }
...
std::vector<rtc::scoped_refptr<RtpSenderInterface>> TgPeerConnection::GetSenders()
    const {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
TgPeerConnection::GetReceivers() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
std::vector<
    rtc::scoped_refptr<RtpReceiverProxyWithInternal<RtpReceiverInternal>>>
TgPeerConnection::GetReceiversInternal() const {
  std::vector<
      rtc::scoped_refptr<RtpReceiverProxyWithInternal<RtpReceiverInternal>>>
      all_receivers;
  for (const auto& transceiver : transceivers_) {
...
TgPeerConnection::ice_connection_state() {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return ice_connection_state_;
}

PeerConnectionInterface::IceConnectionState
TgPeerConnection::standardized_ice_connection_state() {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return standardized_ice_connection_state_;
}

PeerConnectionInterface::PeerConnectionState
TgPeerConnection::peer_connection_state() {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return connection_state_;
}

PeerConnectionInterface::IceGatheringState
TgPeerConnection::ice_gathering_state() {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return ice_gathering_state_;
}

void TgPeerConnection::RestartIce() {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
void TgPeerConnection::DoCreateOffer(
  ...
  if (IsClosed()) {
    std::string error = "CreateOffer called when TgPeerConnection is closed.";
  ...
  // If a session error has occurred the TgPeerConnection is in a possibly
...
void TgPeerConnection::RemoveRecvDirectionFromReceivingTransceiversOfType(
    cricket::MediaType media_type) {
...
void TgPeerConnection::AddUpToOneReceivingTransceiverOfType(
    cricket::MediaType media_type) {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
TgPeerConnection::GetReceivingTransceiversOfType(cricket::MediaType media_type) {
  std::vector<
      rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>>
...
void TgPeerConnection::SetLocalDescription(
      ...
      [this_weak_ptr = weak_ptr_factory_.GetWeakPtr(),
       ...
       desc = std::unique_ptr<SessionDescriptionInterface>(desc_ptr)](
        ...
        if (!this_weak_ptr) {
          ...
          // TODO(hbos): If/when we process SLD messages in ~TgPeerConnection,
        ...
        switch (this_weak_ptr->signaling_state()) {
          ...
          case PeerConnectionInterface::kClosed:
            create_sdp_observer->OnFailure(RTCError(
                ...
                "SetLocalDescription called when TgPeerConnection is closed."));
            break;
        }
      });
}

void TgPeerConnection::DoSetLocalDescription(
    std::unique_ptr<SessionDescriptionInterface> desc,
    rtc::scoped_refptr<SetSessionDescriptionObserver> observer) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  TRACE_EVENT0("webrtc", "TgPeerConnection::DoSetLocalDescription");
  ...
  if (!error.ok()) {
    // If ApplyLocalDescription fails, the TgPeerConnection could be in an
...
RTCError TgPeerConnection::ApplyLocalDescription(
    std::unique_ptr<SessionDescriptionInterface> desc) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_DCHECK(desc);
...
void TgPeerConnection::SetRemoteDescription(
    SetSessionDescriptionObserver* observer,
    SessionDescriptionInterface* desc_ptr) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  // Chain this operation. If asynchronous operations are pending on the chain,
      ...
      [this_weak_ptr = weak_ptr_factory_.GetWeakPtr(),
       ...
       desc = std::unique_ptr<SessionDescriptionInterface>(desc_ptr)](
        ...
        if (!this_weak_ptr) {
          ...
          // case.
          // TODO(hbos): If/when we process SRD messages in ~TgPeerConnection,
          // the consistent thing would be to inform the observer here.
          operations_chain_callback();
          return;
        }
...
void TgPeerConnection::DoSetRemoteDescription(
    std::unique_ptr<SessionDescriptionInterface> desc,
    rtc::scoped_refptr<SetRemoteDescriptionObserverInterface> observer) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  TRACE_EVENT0("webrtc", "TgPeerConnection::DoSetRemoteDescription");
  ...
  if (!error.ok()) {
    // If ApplyRemoteDescription fails, the TgPeerConnection could be in an
    // inconsistent state, so act conservatively here and set the session error
    // so that future calls to SetLocalDescription/SetRemoteDescription fail.
    SetSessionError(SessionError::kContent, error.message());
    std::string error_message =
...
RTCError TgPeerConnection::ApplyRemoteDescription(
    std::unique_ptr<SessionDescriptionInterface> desc) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_DCHECK(desc);
...
void TgPeerConnection::SetAssociatedRemoteStreams(
...
void TgPeerConnection::ProcessRemovalOfRemoteTrack(
    rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
        transceiver,
    std::vector<rtc::scoped_refptr<RtpTransceiverInterface>>* remove_list,
    std::vector<rtc::scoped_refptr<MediaStreamInterface>>* removed_streams) {
...
void TgPeerConnection::RemoveRemoteStreamsIfEmpty(
...
RTCError TgPeerConnection::UpdateTransceiversAndDataChannels(
...
RTCError TgPeerConnection::UpdateTransceiverChannel(
    rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
        transceiver,
...
TgPeerConnection::AssociateTransceiver(cricket::ContentSource source,
                                     SdpType type,
                                     size_t mline_index,
                                     const ContentInfo& content,
...
rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
TgPeerConnection::GetAssociatedTransceiver(const std::string& mid) const {
  RTC_DCHECK(IsUnifiedPlan());
  for (auto transceiver : transceivers_) {
...
rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
TgPeerConnection::GetTransceiverByMLineIndex(size_t mline_index) const {
  RTC_DCHECK(IsUnifiedPlan());
  for (auto transceiver : transceivers_) {
...
rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
TgPeerConnection::FindAvailableTransceiverToReceive(
  ...
  // the same type that were added to the TgPeerConnection by addTrack and are not
...
const cricket::ContentInfo* TgPeerConnection::FindMediaSectionForTransceiver(
    rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
        transceiver,
...
PeerConnectionInterface::RTCConfiguration TgPeerConnection::GetConfiguration() {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
RTCError TgPeerConnection::SetConfiguration(
  ...
  if (IsClosed()) {
    LOG_AND_RETURN_ERROR(RTCErrorType::INVALID_STATE,
                         "SetConfiguration: TgPeerConnection is closed.");
  }
  ...
  if (!network_thread()->Invoke<bool>(
          RTC_FROM_HERE,
          rtc::Bind(&TgPeerConnection::ReconfigurePortAllocator_n, this,
...
bool TgPeerConnection::AddIceCandidate(
...
void TgPeerConnection::AddIceCandidate(
...
bool TgPeerConnection::RemoveIceCandidates(
    const std::vector<cricket::Candidate>& candidates) {
  TRACE_EVENT0("webrtc", "TgPeerConnection::RemoveIceCandidates");
  RTC_DCHECK_RUN_ON(signaling_thread());
  if (IsClosed()) {
    RTC_LOG(LS_ERROR) << "RemoveIceCandidates: TgPeerConnection is closed.";
    return false;
  }
...
RTCError TgPeerConnection::SetBitrate(const BitrateSettings& bitrate) {
...
void TgPeerConnection::SetAudioPlayout(bool playout) {
  if (!worker_thread()->IsCurrent()) {
    worker_thread()->Invoke<void>(
        RTC_FROM_HERE,
        rtc::Bind(&TgPeerConnection::SetAudioPlayout, this, playout));
    return;
  }
  auto audio_state =
...
void TgPeerConnection::SetAudioRecording(bool recording) {
  if (!worker_thread()->IsCurrent()) {
    worker_thread()->Invoke<void>(
        RTC_FROM_HERE,
        rtc::Bind(&TgPeerConnection::SetAudioRecording, this, recording));
    return;
  }
  auto audio_state =
      factory_->channel_manager()->media_engine()->voice().GetAudioState();
...
TgPeerConnection::GetRemoteAudioSSLCertificate() {
...
TgPeerConnection::GetRemoteAudioSSLCertChain() {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
TgPeerConnection::GetFirstAudioTransceiver() const {
  for (auto transceiver : transceivers_) {
    if (transceiver->media_type() == cricket::MEDIA_TYPE_AUDIO) {
      return transceiver;
    }
...
bool TgPeerConnection::StartRtcEventLog(std::unique_ptr<RtcEventLogOutput> output,
...
bool TgPeerConnection::StartRtcEventLog(
...
void TgPeerConnection::StopRtcEventLog() {
  worker_thread()->Invoke<void>(
      RTC_FROM_HERE, rtc::Bind(&TgPeerConnection::StopRtcEventLog_w, this));
}

rtc::scoped_refptr<DtlsTransportInterface>
TgPeerConnection::LookupDtlsTransportByMid(const std::string& mid) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return transport_controller_->LookupDtlsTransportByMid(mid);
}

rtc::scoped_refptr<DtlsTransport>
TgPeerConnection::LookupDtlsTransportByMidInternal(const std::string& mid) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return transport_controller_->LookupDtlsTransportByMid(mid);
}

rtc::scoped_refptr<SctpTransportInterface> TgPeerConnection::GetSctpTransport()
    const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  if (!sctp_mid_) {
    return nullptr;
...
const SessionDescriptionInterface* TgPeerConnection::local_description() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return pending_local_description_ ? pending_local_description_.get()
                                    : current_local_description_.get();
}

const SessionDescriptionInterface* TgPeerConnection::remote_description() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return pending_remote_description_ ? pending_remote_description_.get()
                                     : current_remote_description_.get();
}

const SessionDescriptionInterface* TgPeerConnection::current_local_description()
    const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return current_local_description_.get();
}

const SessionDescriptionInterface* TgPeerConnection::current_remote_description()
    const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return current_remote_description_.get();
}

const SessionDescriptionInterface* TgPeerConnection::pending_local_description()
    const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return pending_local_description_.get();
}

const SessionDescriptionInterface* TgPeerConnection::pending_remote_description()
    const {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
void TgPeerConnection::OnMessage(rtc::Message* msg) {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
cricket::VoiceMediaChannel* TgPeerConnection::voice_media_channel() const {
  RTC_DCHECK(!IsUnifiedPlan());
...
cricket::VideoMediaChannel* TgPeerConnection::video_media_channel() const {
  RTC_DCHECK(!IsUnifiedPlan());
...
void TgPeerConnection::CreateAudioReceiver(
...
void TgPeerConnection::CreateVideoReceiver(
    MediaStreamInterface* stream,
    const RtpSenderInfo& remote_sender_info) {
  std::vector<rtc::scoped_refptr<MediaStreamInterface>> streams;
  streams.push_back(rtc::scoped_refptr<MediaStreamInterface>(stream));
...
// description.
rtc::scoped_refptr<RtpReceiverInterface> TgPeerConnection::RemoveAndStopReceiver(
    const RtpSenderInfo& remote_sender_info) {
...
void TgPeerConnection::AddAudioTrack(AudioTrackInterface* track,
                                   MediaStreamInterface* stream) {
  RTC_DCHECK(!IsClosed());
  RTC_DCHECK(track);
  RTC_DCHECK(stream);
...
void TgPeerConnection::RemoveAudioTrack(AudioTrackInterface* track,
                                      MediaStreamInterface* stream) {
  RTC_DCHECK(!IsClosed());
  auto sender = FindSenderForTrack(track);
  if (!sender) {
...
void TgPeerConnection::AddVideoTrack(VideoTrackInterface* track,
                                   MediaStreamInterface* stream) {
  RTC_DCHECK(!IsClosed());
  RTC_DCHECK(track);
  RTC_DCHECK(stream);
...
void TgPeerConnection::RemoveVideoTrack(VideoTrackInterface* track,
                                      MediaStreamInterface* stream) {
  RTC_DCHECK(!IsClosed());
  auto sender = FindSenderForTrack(track);
  if (!sender) {
...
void TgPeerConnection::SetStandardizedIceConnectionState(
...
void TgPeerConnection::SetConnectionState(
...
void TgPeerConnection::OnIceGatheringChange(
...
void TgPeerConnection::OnIceCandidate(
    std::unique_ptr<IceCandidateInterface> candidate) {
  if (IsClosed()) {
    return;
  }
...
void TgPeerConnection::OnIceCandidateError(const std::string& address,
                                         int port,
...
void TgPeerConnection::OnIceCandidatesRemoved(
    const std::vector<cricket::Candidate>& candidates) {
  if (IsClosed()) {
    return;
  }
...
void TgPeerConnection::OnSelectedCandidatePairChanged(
...
void TgPeerConnection::ChangeSignalingState(
...
void TgPeerConnection::OnAudioTrackAdded(AudioTrackInterface* track,
                                       MediaStreamInterface* stream) {
  if (IsClosed()) {
    return;
  }
...
void TgPeerConnection::OnAudioTrackRemoved(AudioTrackInterface* track,
                                         MediaStreamInterface* stream) {
  if (IsClosed()) {
    return;
  }
...
void TgPeerConnection::OnVideoTrackAdded(VideoTrackInterface* track,
                                       MediaStreamInterface* stream) {
  if (IsClosed()) {
    return;
  }
...
void TgPeerConnection::OnVideoTrackRemoved(VideoTrackInterface* track,
                                         MediaStreamInterface* stream) {
  if (IsClosed()) {
    return;
  }
...
void TgPeerConnection::PostSetSessionDescriptionSuccess(
    SetSessionDescriptionObserver* observer) {
...
void TgPeerConnection::PostSetSessionDescriptionFailure(
    SetSessionDescriptionObserver* observer,
    RTCError&& error) {
  RTC_DCHECK(!error.ok());
  SetSessionDescriptionMsg* msg = new SetSessionDescriptionMsg(observer);
...
void TgPeerConnection::PostCreateSessionDescriptionFailure(
...
void TgPeerConnection::GetOptionsForUnifiedPlanOffer(
  ...
  for (size_t i = 0;
    ...
    if (media_type == cricket::MEDIA_TYPE_AUDIO ||
      ...
      if (had_been_rejected && transceiver->stopped()) {
        ...
        // TgPeerConnection, but we need a way to match new transceivers to new
  ...
  // added in the order they were added to the TgPeerConnection.
  for (const auto& transceiver : transceivers_) {
...
void TgPeerConnection::GenerateMediaDescriptionOptions(
...
void TgPeerConnection::RemoveSenders(cricket::MediaType media_type) {
...
void TgPeerConnection::UpdateRemoteSendersList(
...
void TgPeerConnection::OnRemoteSenderAdded(const RtpSenderInfo& sender_info,
                                         cricket::MediaType media_type) {
...
void TgPeerConnection::OnRemoteSenderRemoved(const RtpSenderInfo& sender_info,
                                           cricket::MediaType media_type) {
...
void TgPeerConnection::UpdateEndedRemoteMediaStreams() {
...
void TgPeerConnection::UpdateLocalSenders(
...
void TgPeerConnection::OnLocalSenderAdded(const RtpSenderInfo& sender_info,
                                        cricket::MediaType media_type) {
  RTC_DCHECK(!IsUnifiedPlan());
...
void TgPeerConnection::OnLocalSenderRemoved(const RtpSenderInfo& sender_info,
                                          cricket::MediaType media_type) {
  auto sender = FindSenderById(sender_info.sender_id);
  if (!sender) {
  ...
  // associated with the TgPeerConnection. This only occurs if the SDP doesn't
...
rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
TgPeerConnection::GetAudioTransceiver() const {
...
rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
TgPeerConnection::GetVideoTransceiver() const {
  // This method only works with Plan B SDP, where there is a single
  // audio/video transceiver.
  RTC_DCHECK(!IsUnifiedPlan());
  for (auto transceiver : transceivers_) {
...
bool TgPeerConnection::HasRtpSender(cricket::MediaType type) const {
  switch (type) {
    case cricket::MEDIA_TYPE_AUDIO:
...
rtc::scoped_refptr<RtpReceiverProxyWithInternal<RtpReceiverInternal>>
TgPeerConnection::FindReceiverById(const std::string& receiver_id) const {
  for (const auto& transceiver : transceivers_) {
...
std::vector<TgPeerConnection::RtpSenderInfo>*
TgPeerConnection::GetRemoteSenderInfos(cricket::MediaType media_type) {
  RTC_DCHECK(media_type == cricket::MEDIA_TYPE_AUDIO ||
             media_type == cricket::MEDIA_TYPE_VIDEO);
...
std::vector<TgPeerConnection::RtpSenderInfo>* TgPeerConnection::GetLocalSenderInfos(
    cricket::MediaType media_type) {
  RTC_DCHECK(media_type == cricket::MEDIA_TYPE_AUDIO ||
             media_type == cricket::MEDIA_TYPE_VIDEO);
...
const TgPeerConnection::RtpSenderInfo* TgPeerConnection::FindSenderInfo(
    const std::vector<TgPeerConnection::RtpSenderInfo>& infos,
    const std::string& stream_id,
...
TgPeerConnection::InitializePortAllocatorResult
...
cricket::ChannelManager* TgPeerConnection::channel_manager() const {
  return factory_->channel_manager();
}

bool TgPeerConnection::StartRtcEventLog_w(
    std::unique_ptr<RtcEventLogOutput> output,
    int64_t output_period_ms) {
  RTC_DCHECK_RUN_ON(worker_thread());
  if (!event_log_) {
...
void TgPeerConnection::StopRtcEventLog_w() {
  RTC_DCHECK_RUN_ON(worker_thread());
  if (event_log_) {
...
cricket::ChannelInterface* TgPeerConnection::GetChannel(
    const std::string& content_name) {
  for (const auto& transceiver : transceivers_) {
    cricket::ChannelInterface* channel = transceiver->internal()->channel();
...
bool TgPeerConnection::GetSctpSslRole(rtc::SSLRole* role) {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
bool TgPeerConnection::GetSslRole(const std::string& content_name,
                                rtc::SSLRole* role) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  if (!local_description() || !remote_description()) {
    RTC_LOG(LS_INFO)
...
void TgPeerConnection::SetSessionError(SessionError error,
...
RTCError TgPeerConnection::UpdateSessionState(
    SdpType type,
    cricket::ContentSource source,
...
RTCError TgPeerConnection::PushdownMediaDescription(
    SdpType type,
    cricket::ContentSource source) {
  const SessionDescriptionInterface* sdesc =
...
RTCError TgPeerConnection::PushdownTransportDescription(
    cricket::ContentSource source,
    SdpType type) {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
bool TgPeerConnection::GetTransportDescription(
...
cricket::IceConfig TgPeerConnection::ParseIceConfig(
...
absl::optional<std::string> TgPeerConnection::sctp_transport_name() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
cricket::CandidateStatsList TgPeerConnection::GetPooledCandidateStats() const {
...
std::map<std::string, std::string> TgPeerConnection::GetTransportNamesByMid()
    const {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
TgPeerConnection::GetTransportStatsByNames(
...
bool TgPeerConnection::GetLocalCertificate(
...
std::unique_ptr<rtc::SSLCertChain> TgPeerConnection::GetRemoteSSLCertChain(
    const std::string& transport_name) {
  return transport_controller_->GetRemoteSSLCertChain(transport_name);
}

bool TgPeerConnection::IceRestartPending(const std::string& content_name) const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  return pending_ice_restarts_.find(content_name) !=
         pending_ice_restarts_.end();
}

bool TgPeerConnection::NeedsIceRestart(const std::string& content_name) const {
  return transport_controller_->NeedsIceRestart(content_name);
}

void TgPeerConnection::OnCertificateReady(
    const rtc::scoped_refptr<rtc::RTCCertificate>& certificate) {
  transport_controller_->SetLocalCertificate(certificate);
}

void TgPeerConnection::OnDtlsSrtpSetupFailure(cricket::BaseChannel*, bool rtcp) {
  SetSessionError(SessionError::kTransport,
                  rtcp ? kDtlsSrtpSetupFailureRtcp : kDtlsSrtpSetupFailureRtp);
}

void TgPeerConnection::OnTransportControllerConnectionState(
...
void TgPeerConnection::OnTransportControllerCandidatesGathered(
    const std::string& transport_name,
...
void TgPeerConnection::OnTransportControllerCandidateError(
    const cricket::IceCandidateErrorEvent& event) {
  OnIceCandidateError(event.address, event.port, event.url, event.error_code,
                      event.error_text);
}

void TgPeerConnection::OnTransportControllerCandidatesRemoved(
    const std::vector<cricket::Candidate>& candidates) {
  // Sanity check.
...
void TgPeerConnection::OnTransportControllerCandidateChanged(
    const cricket::CandidatePairChangeEvent& event) {
  OnSelectedCandidatePairChanged(event);
}

void TgPeerConnection::OnTransportControllerDtlsHandshakeError(
    rtc::SSLHandshakeError error) {
  RTC_HISTOGRAM_ENUMERATION(
      "WebRTC.TgPeerConnection.DtlsHandshakeError", static_cast<int>(error),
      static_cast<int>(rtc::SSLHandshakeError::MAX_VALUE));
}

void TgPeerConnection::EnableSending() {
  for (const auto& transceiver : transceivers_) {
    cricket::ChannelInterface* channel = transceiver->internal()->channel();
...
bool TgPeerConnection::GetLocalCandidateMediaIndex(
    const std::string& content_name,
    int* sdp_mline_index) {
...
bool TgPeerConnection::UseCandidatesInSessionDescription(
...
bool TgPeerConnection::UseCandidate(const IceCandidateInterface* candidate) {
...
RTCErrorOr<const cricket::ContentInfo*> TgPeerConnection::FindContentInfo(
...
void TgPeerConnection::RemoveUnusedChannels(const SessionDescription* desc) {
...
RTCErrorOr<const cricket::ContentGroup*> TgPeerConnection::GetEarlyBundleGroup(
...
RTCError TgPeerConnection::CreateChannels(const SessionDescription& desc) {
...
cricket::VoiceChannel* TgPeerConnection::CreateVoiceChannel(
    const std::string& mid) {
  ...
  voice_channel->SignalDtlsSrtpSetupFailure.connect(
      this, &TgPeerConnection::OnDtlsSrtpSetupFailure);
  voice_channel->SignalSentPacket.connect(this,
                                          &TgPeerConnection::OnSentPacket_w);
...
// TODO(steveanton): Perhaps this should be managed by the RtpTransceiver.
cricket::VideoChannel* TgPeerConnection::CreateVideoChannel(
    const std::string& mid) {
  RtpTransportInternal* rtp_transport = GetRtpTransport(mid);
  MediaTransportConfig media_transport_config =
      transport_controller_->GetMediaTransportConfig(mid);
...
Call::Stats TgPeerConnection::GetCallStats() {
  if (!worker_thread()->IsCurrent()) {
    return worker_thread()->Invoke<Call::Stats>(
        RTC_FROM_HERE, rtc::Bind(&TgPeerConnection::GetCallStats, this));
  }
  RTC_DCHECK_RUN_ON(worker_thread());
  if (call_) {
...
bool TgPeerConnection::ValidateBundleSettings(const SessionDescription* desc) {
...
bool TgPeerConnection::HasRtcpMuxEnabled(const cricket::ContentInfo* content) {
...
RTCError TgPeerConnection::ValidateSessionDescription(
    const SessionDescriptionInterface* sdesc,
    cricket::ContentSource source) {
  if (session_error() != SessionError::kNone) {
    LOG_AND_RETURN_ERROR(RTCErrorType::INTERNAL_ERROR, GetSessionErrorMsg());
...
bool TgPeerConnection::ExpectSetLocalDescription(SdpType type) {
...
bool TgPeerConnection::ExpectSetRemoteDescription(SdpType type) {
  PeerConnectionInterface::SignalingState state = signaling_state();
  if (type == SdpType::kOffer) {
    return (state == PeerConnectionInterface::kStable) ||
...
const char* TgPeerConnection::SessionErrorToString(SessionError error) const {
  switch (error) {
...
std::string TgPeerConnection::GetSessionErrorMsg() {
...
void TgPeerConnection::ReportSdpFormatReceived(
...
void TgPeerConnection::ReportIceCandidateCollected(
...
void TgPeerConnection::ReportRemoteIceCandidateAdded(
    const cricket::Candidate& candidate) {
...
void TgPeerConnection::NoteUsageEvent(UsageEvent event) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  usage_event_accumulator_ |= static_cast<int>(event);
}

void TgPeerConnection::ReportUsagePattern() const {
  RTC_DLOG(LS_INFO) << "Usage signature is " << usage_event_accumulator_;
  RTC_HISTOGRAM_ENUMERATION_SPARSE("WebRTC.TgPeerConnection.UsagePattern",
...
void TgPeerConnection::ReportNegotiatedSdpSemantics(
...
bool TgPeerConnection::ReadyToUseRemoteCandidate(
    const IceCandidateInterface* candidate,
    const SessionDescriptionInterface* remote_desc,
    bool* valid) {
  *valid = true;
...
bool TgPeerConnection::SrtpRequired() const {
  return !use_datagram_transport_ &&
         (dtls_enabled_ ||
          webrtc_session_desc_factory_->SdesPolicy() == cricket::SEC_REQUIRED);
}

void TgPeerConnection::OnTransportControllerGatheringState(
...
void TgPeerConnection::ReportTransportStats() {
...
// for IPv4 and IPv6.
void TgPeerConnection::ReportBestConnectionState(
...
void TgPeerConnection::ReportNegotiatedCiphers(
    const cricket::TransportStats& stats,
...
void TgPeerConnection::OnSentPacket_w(const rtc::SentPacket& sent_packet) {
  RTC_DCHECK_RUN_ON(worker_thread());
  RTC_DCHECK(call_);
  call_->OnSentPacket(sent_packet);
}

const std::string TgPeerConnection::GetTransportName(
    const std::string& content_name) {
...
void TgPeerConnection::DestroyTransceiverChannel(
    rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
        transceiver) {
  RTC_DCHECK(transceiver);
...
void TgPeerConnection::DestroyChannelInterface(
...
bool TgPeerConnection::OnTransportChanged(
    const std::string& mid,
...
void TgPeerConnection::OnSetStreams() {
  RTC_DCHECK_RUN_ON(signaling_thread());
  if (IsUnifiedPlan())
    UpdateNegotiationNeeded();
}

PeerConnectionObserver* TgPeerConnection::Observer() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_DCHECK(observer_);
  return observer_;
}

CryptoOptions TgPeerConnection::GetCryptoOptions() {
...
void TgPeerConnection::UpdateNegotiationNeeded() {
  RTC_DCHECK_RUN_ON(signaling_thread());
  if (!IsUnifiedPlan()) {
    Observer()->OnRenegotiationNeeded();
    return;
...
bool TgPeerConnection::CheckIfNegotiationIsNeeded() {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
RTCError TgPeerConnection::Rollback(SdpType sdp_type) {
