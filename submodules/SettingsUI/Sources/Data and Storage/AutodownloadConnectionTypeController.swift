...
private func autodownloadMediaConnectionTypeControllerEntries(presentationData: PresentationData, connectionType: AutomaticDownloadConnectionType, settings: MediaAutoDownloadSettings) -> [AutodownloadMediaCategoryEntry] {
    ...
    if #available(iOSApplicationExtension 13.0, iOS 13.0, *) {
        entries.append(.lowDataMode(presentationData.theme, presentationData.strings.AutoDownloadSettings_LowDataMode, true))
        entries.append(.lowDataModeInfo(presentationData.theme, presentationData.strings.AutoDownloadSettings_LowDataModeInfo))
    }
