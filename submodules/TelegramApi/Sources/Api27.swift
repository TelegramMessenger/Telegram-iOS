...
public extension Api {
    enum User: TypeConstructorDescription {
        case user(flags: Int32, flags2: Int32, id: Int64, accessHash: Int64?, firstName: String?, lastName: String?, username: String?, phone: String?, photo: Api.UserProfilePhoto?, status: Api.UserStatus?, botInfoVersion: Int32?, restrictionReason: [Api.RestrictionReason]?, botInlinePlaceholder: String?, langCode: String?, emojiStatus: Api.EmojiStatus?, usernames: [Api.Username]?, storiesMaxId: Int32?, color: Api.PeerColor?, profileColor: Api.PeerColor?, botActiveUsers: Int32?, botVerification: Api.BotVerification?)
        case userEmpty(id: Int64)
    
    public func serialize(_ buffer: Buffer, _ boxed: Swift.Bool) {
    switch self {
                case .user(let flags, let flags2, let id, let accessHash, let firstName, let lastName, let username, let phone, let photo, let status, let botInfoVersion, let restrictionReason, let botInlinePlaceholder, let langCode, let emojiStatus, let usernames, let storiesMaxId, let color, let profileColor, let botActiveUsers, let botVerification):
                    if boxed {
                        buffer.appendInt32(1497852246)
                    }
                    serializeInt32(flags, buffer: buffer, boxed: false)
                    ...
                    if Int(flags2) & Int(1 << 14) != 0 {botVerification!.serialize(buffer, true)}
                    break
    ...
    public func descriptionFields() -> (String, [(String, Any)]) {
        ...
        public static func parse_user(_ reader: BufferReader) -> User? {
            ...
            var _21: Api.BotVerification?
            if Int(_2!) & Int(1 << 14) != 0 {if let signature = reader.readInt32() {
                _21 = Api.parse(reader, signature: signature) as? Api.BotVerification
            } }
            let _c1 = _1 != nil
            let _c2 = _2 != nil
            let _c3 = _3 != nil
