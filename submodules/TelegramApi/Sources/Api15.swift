...
public extension Api {
    enum MessageAction: TypeConstructorDescription {
        ...
        case messageActionPaidMessage(stars: Int64)
    ...
    switch self {
                ...
                case .messageActionInviteToGroupCall(let call, let users):
                    ...
                    break
                case .messageActionPaidMessage(let stars):
                    if boxed {
                        buffer.appendInt32(1557286943)
                    }
                    serializeInt64(stars, buffer: buffer, boxed: false)
                    break
    ...
    public func descriptionFields() -> (String, [(String, Any)]) {
        switch self {
                ...
                case .messageActionPaidMessage(let stars):
                return ("messageActionPaidMessage", [("stars", stars as Any)])
                case .messageActionPaymentRefunded(let flags, let peer, let currency, let totalAmount, let payload, let charge):
        ...
        public static func parse_messageActionPaidMessage(_ reader: BufferReader) -> MessageAction? {
            var _1: Int64?
            _1 = reader.readInt64()
            let _c1 = _1 != nil
            if _c1 {
                return Api.MessageAction.messageActionPaidMessage(stars: _1!)
            }
            else {
                return nil
            }
