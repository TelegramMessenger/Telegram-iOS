...
public class ChatListControllerImpl: TelegramBaseController, ChatListController {
    ...
    override public func loadDisplayNode() {
        ...
        let tabContextGesture: (Int32?, ContextExtractedContentContainingNode, ContextGesture, Bool, Bool) -> Void = { [weak self] id, sourceNode, gesture, keepInPlace, isDisabled in
            ...
            |> mapToSignal { filters -> Signal<(areMuted: Bool, peerIds: [EnginePeer.Id])?, NoError> in
                ...
                return context.engine.peers.getChatListPeers(filterPredicate: filterPredicate, inactiveSecretChatPeerIds: context.inactiveSecretChatPeerIds)
    ...
    func openArchiveMoreMenu(sourceView: UIView, gesture: ContextGesture?) {
        ...
        let _ = (
            self.context.engine.messages.chatList(group: .archive, count: 10, inactiveSecretChatPeerIds: self.context.inactiveSecretChatPeerIds) |> take(1)
    ...
    func archiveChats(peerIds: [PeerId]) {
        ...
        let hasArchived = engine.messages.chatList(group: .archive, count: 10, inactiveSecretChatPeerIds: self.context.inactiveSecretChatPeerIds)
        |> take(1)
        |> map { list -> Bool in
