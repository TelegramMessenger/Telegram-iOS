...
class QuickReplyItem: ListViewItem, ItemListItem {
    ...
    init(presentationData: ItemListPresentationData, id: String, placeholder: String, value: String, isSelected: Bool?, maxLength: Int?, sectionId: ItemListSectionId, updated: @escaping (String, Bool) -> Void, delete: @escaping (Bool) -> Void, canDelete: Bool, canMove: Bool, focused: @escaping (Bool) -> Void, toggleSelected: @escaping () -> Void, tag: ItemListItemTag?) {
    ...
    func updateNode(async: @escaping (@escaping () -> Void) -> Void, node: @escaping () -> ListViewItemNode, params: ListViewItemLayoutParams, previousItem: ListViewItem?, nextItem: ListViewItem?, animation: ListViewItemUpdateAnimation, completion: @escaping (ListViewItemNodeLayout, @escaping (ListViewItemApply) -> Void) -> Void) {
        Queue.mainQueue().async {
            if let nodeValue = node() as? QuickReplyItemNode {
...
class QuickReplyItemNode: ItemListRevealOptionsItemNode, ItemListItemNode, ItemListItemFocusableNode, ASEditableTextNodeDelegate {
    ...
    private var item: QuickReplyItem?
    ...
    func asyncLayout() -> (_ item: QuickReplyItem, _ params: ListViewItemLayoutParams, _ neighbors: ItemListNeighbors) -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation) -> Void) {
...
//            let insets = itemListNeighborsGroupedInsets(neighbors, params)
            ...
            let displayTextLimit: Bool
            let remainingCount: Int
            if let maxLength = item.maxLength {
                displayTextLimit = textLength > maxLength * 70 / 100
                remainingCount = maxLength - textLength
            } else {
                displayTextLimit = false
                remainingCount = 0
            }
